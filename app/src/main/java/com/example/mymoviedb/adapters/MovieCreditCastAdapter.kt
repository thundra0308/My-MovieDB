...
import com.example.mymoviedb.models.MovieCreditCastModel
import com.example.mymoviedb.utils.Constants
import com.flaviofaria.kenburnsview.KenBurnsView

class MovieCreditCastAdapter(private val context: Context, private val creditList :List<MovieCreditCastModel>): RecyclerView.Adapter<MovieCreditCastAdapter.MainViewHolder>() {
    private lateinit var mListener: onItemClickListener
    inner class MainViewHolder(private var itemView: View, private val listener: onItemClickListener): RecyclerView.ViewHolder(itemView) {
        init {
            itemView.setOnClickListener {
                listener.onItemClick(adapterPosition)
            }
        }
        fun bindData(creditCast: MovieCreditCastModel) {
            ...
            Glide
                .with(context)
                .load(Constants.IMAGE_BASE_URL+creditCast.profile_path)
                .centerCrop()
                .placeholder(R.drawable.ic_profileplaceholder)
                .dontAnimate()
                .into(image)
            name.text = creditCast.name
            characterName.text = "Character : ${creditCast.character}"
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MainViewHolder {
        val view = LayoutInflater.from(context).inflate(R.layout.rvitem_movie_credit,parent,false)
